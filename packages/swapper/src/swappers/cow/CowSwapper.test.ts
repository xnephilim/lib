...
import { BTC, ETH, JINX, WBTC, WETH } from '../utils/test-data/assets'
...
const ASSET_IDS = [ETH.assetId, WBTC.assetId, WETH.assetId, BTC.assetId, JINX.assetId]
...
describe('CowSwapper', () => {
  ...
  describe('getUsdRate', () => {
    it('calls getUsdRate on swapper.getUsdRate', async () => {
      await swapper.getUsdRate(JINX)
      expect(getUsdRate).toHaveBeenCalledWith(COW_SWAPPER_DEPS, JINX)
    })
  })
  ...
  describe('filterAssetIdsBySellable', () => {
    ...
    it('returns array filtered out of unsupported tokens', () => {
      const assetIds = [JINX.assetId, 'eip155:1/erc20:0xdc49108ce5c57bc3408c3a5e95f3d864ec386ed3']
      expect(swapper.filterAssetIdsBySellable(assetIds)).toEqual([JINX.assetId])
    })
  })
  ...
  describe('filterBuyAssetsBySellAssetId', () => {
    ...
    it('returns array filtered out of non erc20 tokens when called with a sellable sellAssetId', () => {
      ...
      ).toEqual([ETH.assetId, WBTC.assetId, JINX.assetId])
      expect(
        swapper.filterBuyAssetsBySellAssetId({
          assetIds: ASSET_IDS,
          sellAssetId: WBTC.assetId,
        }),
      ).toEqual([ETH.assetId, WETH.assetId, JINX.assetId])
      expect(
        swapper.filterBuyAssetsBySellAssetId({
          assetIds: ASSET_IDS,
          sellAssetId: JINX.assetId,
        }),
    ...
    it('returns array filtered out of unsupported tokens when called with a sellable sellAssetId', () => {
      ...
      ).toEqual([JINX.assetId])
      expect(swapper.filterBuyAssetsBySellAssetId({ assetIds, sellAssetId: JINX.assetId })).toEqual(
        [],
      )
    })
  })
