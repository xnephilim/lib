...
import { BTC, ETH, JINX, USDC, WBTC } from '../../../utils/test-data/assets'
...
describe('utils', () => {
  ...
  describe('getUsdRate', () => {
    it('gets the usd rate of JINX', async () => {
      ...
      const rate = await getUsdRate(cowSwapperDeps, JINX)
    ...
    it('should fail when api is returning 0 as token amount', async () => {
      ...
      await expect(getUsdRate(cowSwapperDeps, JINX)).rejects.toThrow(
    ...
    it('should fail when axios is throwing an error', async () => {
      ...
      await expect(getUsdRate(cowSwapperDeps, JINX)).rejects.toThrow('[getUsdRate]')
    })
  })
