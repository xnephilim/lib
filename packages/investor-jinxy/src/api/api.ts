...
export class JinxyApi {
  ...
  public web3: Web3
  private jinxyStakingContracts: Contract[]
  ...
  constructor({
    ...
    this.jinxyStakingContracts = jinxyAddresses.map(
      (addresses) => new this.web3.eth.Contract(jinxyStakingAbi, addresses.staking),
    )
  ...
  async getJinxyOpportunityByStakingAddress(stakingAddress: string) {
    ...
    const addresses = this.jinxyAddresses.find(async (item) => {
  ...
  async tvl(input: TokenAddressInput): Promise<BigNumber> {
    ...
    this.verifyAddresses([tokenContractAddress])
    const contract = new this.web3.eth.Contract(jinxyAbi, tokenContractAddress)
  ...
  async getRebaseHistory(input: BalanceInput) {
    ...
    const jinxyContract = new this.web3.eth.Contract(jinxyAbi, tokenContractAddress)
    const fromBlock = 14381454 // genesis rebase

    const rebaseEvents = await (async () => {
      try {
        const events = (
          await jinxyContract.getPastEvents('LogRebase', {
            fromBlock,
            toBlock: 'latest',
          })
    ...
    const transferEvents = await (async () => {
      try {
        const events = await jinxyContract.getPastEvents('Transfer', {
          fromBlock,
          toBlock: 'latest',
        })
        return events
      ...
      events.map(async (event) => {
        const { preRebaseBalance, postRebaseBalance } = await (async () => {
          try {
            ...
            const postRebaseBalanceResult = await jinxyContract.methods
              .balanceOf(userAddress)
              .call(null, event.blockNumber)
            const unadjustedPreRebaseBalance = await jinxyContract.methods
              .balanceOf(userAddress)
